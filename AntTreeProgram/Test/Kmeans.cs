...
namespace AntTreeProgram
{
    class Kmeans
    {
        public static double [][] PrepareData(List<Ant> list)
        {
        ...
        static List<AntBranch> AddToBranch(List<Ant> list, int[] clustering, int number)
        {
            List<AntBranch> branches = CreateBranches(number);
            int index = 0;
            foreach (int i in clustering)
            {
                branches[i].Ants.Add(list[index]);
                index++;
            }
            return branches;
        }
        static List<AntBranch> CreateBranches(int number)
        {
            Colors color = new Colors();
            List<AntBranch> branches = new List<AntBranch>();
            while(number>0)
            {
                branches.Add(new AntBranch() { AntColor=color.GetColor(number)});
                number--;
            }
            return branches;
        ...
        public static List<AntBranch> KMeans(List<Ant> list, int groupNumber)
        {
            double[][] rawData = PrepareData(list);
            int numClusters = groupNumber;
            int[] clustering = Cluster(rawData, numClusters); // this is it
            return AddToBranch(list, clustering, numClusters);
